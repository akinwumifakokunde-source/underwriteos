...
// Resolve ?returnTo= to a safe same-origin path, else "/applications".
//
...
export function safeReturnTo() {
  ...
  try {
    ...
    if (url.origin !== window.location.origin) return "/applications";
    ...
    if (!path.startsWith("/") || path.startsWith("//") || path.includes("\\")) return "/applications";
    return path;
  } catch {
    return "/applications";
  }
