...
export default function Hero() {
  return (
    <section className="relative overflow-hidden border-b border-[#eceef1]">
      ...
      <div className="absolute inset-0 bg-gradient-to-b from-[#f3f8f6] via-white to-white" />
      <motion.div
        aria-hidden
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-10 lg:gap-14 items-center">
          <div>
            <motion.div
              ...
              initial="hidden"
              animate="show"
              className="inline-flex items-center gap-2 text-[11px] font-medium text-[#0a2e2a] mb-5 bg-white/70 backdrop-blur border border-[#0d9488]/20 rounded-full pl-1.5 pr-3 py-1 shadow-sm"
            >
            ...
            <motion.h1
              ...
              initial="hidden"
              animate="show"
              className="text-[2.1rem] sm:text-[3.4rem] font-semibold tracking-tight text-[#0a0c12] leading-[1.1] sm:leading-[1.05]"
            >
            ...
            >
              {VALUE_PROPS.map((v) => (
                <div key={v.title} className="rounded-xl border border-[#eceef1] bg-white/70 backdrop-blur px-3 py-2.5">
                  <div className="text-[12px] font-semibold text-[#0a0c12]">{v.title}</div>
                  <div className="text-[11px] text-[#8a909c] mt-0.5 leading-snug">{v.desc}</div>
                </div>
              ))}
            </motion.div>
            ...
            >
              <Link
                to="/onboarding"
                className="group inline-flex items-center gap-1.5 text-sm font-medium text-white bg-[#0a0c12] px-5 py-3 rounded-full hover:bg-[#1c1f26] transition-all shadow-sm hover:shadow-md hover:-translate-y-0.5"
              >
                Start underwriting <ArrowRight className="w-4 h-4 group-hover:translate-x-0.5 transition-transform" />
              </Link>
              <Link
                to="/features"
                className="group inline-flex items-center gap-1.5 text-sm font-medium text-[#0a0c12] bg-white border border-[#e6e8eb] px-5 py-3 rounded-full hover:bg-[#f7f8fa] hover:border-[#d0d3d8] transition-all"
              >
            ...
            <motion.div
              ...
              initial="hidden"
              animate="show"
              className="mt-10 sm:mt-8 flex flex-wrap items-center gap-x-2 gap-y-1.5 text-[11px] font-mono uppercase tracking-wider text-[#8a909c]"
            >
